"""
Factory for creating various types of retrievers for Kubernetes data.

This module provides a centralized way to create and configure different
retrieval strategies including advanced methods like BM25, multi-query,
parent-document, contextual compression, and ensemble retrieval.
"""
...
# Try to import Cohere reranker, fall back to alternatives if not available
try:
...
class K8sRetrieverFactory:
    """Factory for creating different types of retrievers for K8s data."""
    ...
    def _create_multi_query_retriever(self, k: int, **kwargs) -> BaseRetriever:
        ...
        # Configure multi-query parameters
    ...
    def _create_contextual_compression_retriever(self, k: int, **kwargs) -> BaseRetriever:
        ...
        cohere_api_key = os.getenv("COHERE_API_KEY") or "INSERT COHERE API KEY "
        if COHERE_AVAILABLE and cohere_api_key and CohereRerank:
            try:
    ...
    def _create_llm_chain_filter(self):
        ...
        prompt = PromptTemplate(
            template="""Given the following question and context, determine if the context is relevant to the question.
            
Question: {question}
Context: {context}

Is this context relevant? Answer 'YES' if relevant, 'NO' if not relevant.
Answer:""",
